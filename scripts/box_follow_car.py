...
class PandaVisionCommander(Node):
    def __init__(self):
        ...
        self.current_xyz = [0.45, 0.2, 0.45]
    ...
    def control_loop(self):
        if self.state == "SEARCH":
            if not self.target_found:
                ...
                self.current_xyz[1] = -0.03 + 0.15 * math.sin(self.search_angle)
                self.current_xyz[2] = 0.25
    ...
    def execute_grasp_sequence(self):
            ...
            grasp_offset_x = -0.05  # 示例：相机在夹爪前方 4.5 厘米
            grasp_offset_y = 0.020  # 示例：左右偏差
            grasp_offset_z = 0.13  # 抓取时的 Z 高度 (0.05~0.06 通常刚好)
            ...
            # 第一步：先下降到接近高度
            self.send_arm_pose([final_x, final_y, -0.1], 2.0)
            time.sleep(2.0)
